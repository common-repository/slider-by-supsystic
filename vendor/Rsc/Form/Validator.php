...
class RscSsl_Form_Validator
{
     ...
     * @param RscSsl_Common_Collection|array $rules
     * @param RscSsl_Common_Collection|array $filters
     */
    public function __construct(RscSsl_Http_Request $request = null, $method = null, array $rules = array(), array $filters = array())
    {
        $this->request = ($request === null ? RscSsl_Http_Request::create() : $request);
     ...
     * @return bool
     * @throws RscSsl_Form_Validator_EmptyRequestException If HTTP request is not specified
     */
    public function isValid()
    {
        if (!$this->request) {
            throw new RscSsl_Form_Validator_EmptyRequestException('You must submit the request via ' . __CLASS__ . '::setRequest() method');
        }

        /** @var RscSsl_Http_Parameters $method */
     ...
     * @throws RscSsl_Form_Validator_InvalidMethodException If specified method is wrong
     * @return RscSsl_Form_Validator
     */
    public function setMethod($method)
    {
        ...
        if (!in_array($method, array(self::METHOD_GET, self::METHOD_POST))) {
            throw new RscSsl_Form_Validator_InvalidMethodException('Invalid method. Type of the method can only be POST or GET (QUERY)');
        }
     ...
     */
    protected function applyFilters(RscSsl_Http_Parameters $method)
    {
        if (empty($this->filters)) {
            return;
        }

        /** @var RscSsl_Form_Filter_Interface $filter */
     ...
     * @param RscSsl_Http_Parameters $method
     */
    protected function doValidation(RscSsl_Http_Parameters $method)
    {
        if (empty($this->rules)) {
            return;
        }

        /** @var RscSsl_Form_Rule_Interface $rule */
