...
class RscSsl_Dispatcher 
{
    ...
    /**
     * Constructor.
     * @param RscSsl_Environment $environment Plugin environment.
     */
    ...
    public function on($action, $function, $priority = 10, $args = 1)
    {
        if (!is_callable($function)) {
            throw new InvalidArgumentException(sprintf(
                'Argument 2 passed to RscSsl_Dispatcher::on() must be a callable, %s given.',
                gettype($function)
            ));
        }
	...
	public function filter($action, $function, $priority = 10, $args = 1)
    {
        if (!is_callable($function)) {
            throw new InvalidArgumentException(sprintf(
                'Argument 2 passed to RscSsl_Dispatcher::filter() must be a callable, %s given.',
                gettype($function)
            ));
        }
