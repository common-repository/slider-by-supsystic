...
class SupsysticSlider
{
    ...
    {
        if (!class_exists('RscSsl_Autoloader', false)) {
            require dirname(dirname(__FILE__)) . '/vendor/Rsc/Autoloader.php';
            RscSsl_Autoloader::register();
        }

        if (!class_exists('Twig_supTwgSsl_Autoloader', false)) {
            require dirname(dirname(__FILE__)) . '/vendor/Twig/Autoloader.php';
            Twig_supTwgSsl_Autoloader::register();
        }
        ...
        $environment = new RscSsl_Environment('ssl', '1.8.11', $pluginPath);
    ...
    protected function initFilesystem()
    {
        $directories = array(
            ...
            'Twig_supTwgSsl_cache' => '/supsystic-slider/cache/twig',
    ...
    {
        $environment = RscSsl_Environment::ENV_PRODUCTION;

        if (defined('WP_DEBUG') && WP_DEBUG) {
            if (defined('SUPSYSTIC_SLIDER_DEBUG') && SUPSYSTIC_SLIDER_DEBUG) {
                $environment = RscSsl_Environment::ENV_DEVELOPMENT;
            }
        }
