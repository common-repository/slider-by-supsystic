 ...
 */
class SupsysticSlider_Slider_Model_Settings extends SupsysticSlider_Core_BaseModel implements RscSsl_Environment_AwareInterface
{
    ...
    public function getPosts($sliderId = '', $thumbSize = 'thumbnail') {
        ...
        if($elements && !empty($elements)) {
            foreach($elements as $id) {
                ...
                $imageUrl = !empty($imageUrl[0]) ? $imageUrl[0] : '';
    ...
    public function deletePost($sliderId = '', $posts = null) {
        $elements = get_option('post' . $sliderId);
        $elements = unserialize($elements);
