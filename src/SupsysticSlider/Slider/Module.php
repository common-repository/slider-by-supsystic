...
class SupsysticSlider_Slider_Module extends SupsysticSlider_Core_BaseModule
{
	...
	public function render($attributes)
	{
		...
		if(!isset($attributes['image-list'])) {
			...
			$slider->posts = array();
            $slider->images = $attributes['image-list'];
	...
	public function addTwigFunctions() {
      ...
      $twig = $this->getTwig();
		...
		$twig->addFunction(
		       new Twig_supTwgSsl_SimpleFunction('unserialize_twig', array($this, 'unserialize'))
		);
		$twig->addFunction(
		       new Twig_supTwgSsl_SimpleFunction('all_posts', array($this, 'get_posts'))
		);
      $twig->addFunction(
		       new Twig_supTwgSsl_SimpleFunction('all_pages', array($this, 'get_pages'))
      );
      $twig->addFunction(
         new Twig_supTwgSsl_SimpleFunction('all_categories', array($this, 'get_categories'))
      );
      $twig->addFunction(
         new Twig_supTwgSsl_SimpleFunction('get_image_src', array($this, 'wp_get_attachment_image_src'))
      );
   ...
   public function wp_get_attachment_image_src($attachment_id, $size = 'thumbnail', $icon = false) {
    ...
    {
        $twig = $this->getEnvironment()->getTwig();
        $twig->addExtension(new SupsysticSlider_Slider_Twig_supTwgSsl_Attachment());
    }
    ...
    public function addNewSliderMenuItem()
    {
        ...
        $submenuNewSlider->setCapability($capability)
			...
			->setMenuSlug('supsystic-slider&module=slider&action=showPresets&nonce='.$this->getEnvironment()->getNonce())
...
//            ->register();
        ...
        $submenuSliders->setCapability($capability)
            ->setMenuSlug('supsystic-slider&module=slider&nonce='.$this->getEnvironment()->getNonce())
