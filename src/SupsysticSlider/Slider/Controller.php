...
class SupsysticSlider_Slider_Controller extends SupsysticSlider_Core_BaseController
{
    ...
    public function __construct(
        RscSsl_Environment $environment,
        RscSsl_Http_Request $request
    ) {
	...
	public function showPresetsAction(RscSsl_Http_Request $request) {
     ...
     * @return RscSsl_Http_Response
     */
    public function createAction(RscSsl_Http_Request $request)
    {
        if (!$this->_checkNonce($request)) die();
        try {
            /** @var SupsysticSlider_Slider_Model_Sliders $sliders */
        ...
        return $this->response(
            ...
            $this->getSuccessResponseData(
                ...
                array(
                    'title' => $title,
                    'id'    => $sliderId,
                    'url'   => $this->generateUrl('slider', 'view', array('id' => $sliderId, 'nonce' => wp_create_nonce('ssl_nonce'))),
                )));
    }
     ...
     * @return RscSsl_Http_Response
     */
    public function getPreviewAction(RscSsl_Http_Request $request)
    {
        if (!$this->_checkNonce($request)) die();
    ...
    public function getRenderAction(RscSsl_Http_Request $request) {
        if (!$this->_checkNonce($request)) die();
        $sliderId = $request->post->get('id');
        $isEmpty  = false;
    ...
    public function addPostAction(RscSsl_Http_Request $request) {
        if (!$this->_checkNonce($request)) die();
    ...
    public function getPostsAction(RscSsl_Http_Request $request) {
        if (!$this->_checkNonce($request)) die();
        $sliderId = $request->post->get('slider');
        $settings = $this->getModel('settings');
    ...
    public function deletePostsAction(RscSsl_Http_Request $request) {
        if (!$this->_checkNonce($request)) die();
        $sliderId = $request->post->get('slider');
        $settings = $this->getModel('settings');
     ...
     * @return RscSsl_Http_Response
     */
    public function importAction(RscSsl_Http_Request $request)
    {
        if (!$this->_checkNonce($request)) die();
        ...
        $redirectUri = $this->generateUrl('slider', 'view', array('id' => $sliderId, 'nonce' => wp_create_nonce('ssl_nonce')));
     ...
     * @return RscSsl_Http_Response
     */
    public function settingsAction(RscSsl_Http_Request $request)
    {
     ...
     * @return RscSsl_Http_Response
     */
    public function saveSettingsAction(RscSsl_Http_Request $request)
    {
        if (!$this->_checkNonce($request)) die();
        ...
        return $this->redirect($this->generateUrl('slider', 'view', array('id' => $id, 'nonce' => wp_create_nonce('ssl_nonce'))));
    }
     ...
     * @return RscSsl_Http_Response
     */
    public function changePluginAction(RscSsl_Http_Request $request)
    {
        if (!$this->_checkNonce($request)) die();
        $sliders = $this->getSliders();
    ...
    public function changePluginNameAction(RscSsl_Http_Request $request)
    {
        if (!$this->_checkNonce($request)) die();
        $sliders = $this->getSliders();
    ...
    public function importSettingsAction(RscSsl_Http_Request $request)
    {
        if (!$this->_checkNonce($request)) die();
        $sliders = $this->getSliders();
        $settings = $this->getSettings();
     ...
     *
     * @param RscSsl_Http_Request $request [description]
     */
    public function attachAction(RscSsl_Http_Request $request)
    {
        if (!$this->_checkNonce($request)) die();
        $sliderId = $request->post->get('slider_id');
    ...
    public function checkSettingsAction(RscSsl_Http_Request $request) {
        if (!$this->_checkNonce($request)) die();
        $sliderId  = $request->post->get('id');
    ...
    public function getFontsAction(RscSsl_Http_Request $request) {
        if (!$this->_checkNonce($request)) die();
        $resources = $this->getResources();
        $fonts = $resources->getFontsList();
	...
	public function reviewNoticeResponseAction(RscSsl_Http_Request $request) {
        if (!$this->_checkNonce($request)) die();
    ...
    public function deleteResourceAction(RscSsl_Http_Request $request)
    {
        if (!$this->_checkNonce($request)) die();
        $sliderId  = $request->post->get('id');
        $entities  = $request->post->get('resources');
    ...
    public function getSidebarAction(RscSsl_Http_Request $request)
    {
        if (!$this->_checkNonce($request)) die();
        $sliderId = $request->post->get('slider_id');
        ...
        } catch (Twig_supTwgSsl_Error $error) {
            $template = sprintf(
    ...
    public function updateVisualAction(RscSsl_Http_Request $request)
    {
        if (!$this->_checkNonce($request)) die();
        $sliderId = $request->post->get('slider_id');
